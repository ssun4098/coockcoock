...
public class CommandMemberController {
    ...
    @PostMapping("/login")
    @ResponseStatus(HttpStatus.OK)
    public CommonResponseDto<Void> login(@Valid @RequestBody LoginRequestDto requestDto, HttpServletResponse response) {
        response.addCookie(commandMemberService.login(requestDto).getCookie());
